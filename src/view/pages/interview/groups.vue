      ...
      <my-table :showData="showData" :gridData="gridData">
        <el-table-column :label="$t('INTERVIEW_OPERATE')">
          <template slot-scope="scope">
            <div>
              <el-button type="text" @click="Operate(scope.row)">{{$t('INTERVIEW_DETAIL')}}</el-button>
            </div>
          </template>
        </el-table-column>
      </my-table>
...
export default {
  ...
  data() {
    return {
      ...
      showData: [
        ...
        { type: "string", prop: "name", label: "INTERVIEW_GROUP" },
        { type: "string", prop: "number", label: "INTERVIEW_APPLICANTS" }
      ...
      gridData: [
        ...
        { id: 2, name: "二年级", number: 30 },
        { id: 3, name: "三年级", number: 30 },
        { id: 4, name: "四年级", number: 30 },
        { id: 5, name: "五年级", number: 30 }
      ],
      status: ""
    };
  },
  methods: {
    Operate(row) {
      this.$router.push("/interview?p=group&g=" + row.id);
    }
  }
};
</script>
